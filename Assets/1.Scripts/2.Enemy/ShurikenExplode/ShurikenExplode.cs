...
public partial class ShurikenExplode : MonoBehaviour
{
    ...
    */
    ...
    private AudioClipPlus InitAudioClipPlus(UnityEngine.Object raw)
    {
        ...
        AudioClipPlus a = AudioClipPlus.FromClipOrScriptableWithExplanation(raw, out bool isNull, out bool isAudioClip, out bool isAudioClipPlusScriptable);
        if (a == null)
        {
            if (isNull)
            {
                throw new Exception("Obejct for AudioClipPlus is Null");
            }
            else if (!isAudioClip && !isAudioClipPlusScriptable)
            {
                throw new Exception("Obejct for AudioClipPlus is neither AudioClip nor AudioClipPlusScriptable");
            }
            else
            {
                throw new Exception("Obejct for AudioClipPlus is invalid, but the reason is ambiguous.");
            }
        }
        return a;
    }
