...
namespace Ludo.TwoHandsWar
{
    ...
    {   
        void FixedUpdate()
        {
            Logger.Log($"🔻 Fixed Update {Time.fixedTime}");

            FU_Pre();

            FU_Main();

            armGesture.FixedUpdate();
            represent.FixedUpdate();
        ...
        void FU_Main()
        {
            ...
            {
                Logger.Log($"--Left");
                Logger.Log($"leftSpeed: {leftFistVelocity.speed}");
                Logger.Log($"leftOffset: ({fistOffset.left.x}, {fistOffset.left.y})");
                Logger.Log($"leftRelativePositon: ({leftFist.transform.localPosition.x}, {leftFist.transform.localPosition.y})");

                Logger.Log($"--Right");
                Logger.Log($"rightSpeed: {rightFistVelocity.speed}");
                Logger.Log($"rightOffset: ({fistOffset.right.x}, {fistOffset.right.y})");
                Logger.Log($"rightRelativePositon: ({rightFist.transform.localPosition.x}, {rightFist.transform.localPosition.y})");

                Logger.Log($"--Whole");
                Logger.Log($"Whole Velocity Before Jump, speed:{wholeVelocityBeforeJump.speed}, direction: ({wholeVelocityBeforeJump.direction.x}, {wholeVelocityBeforeJump.direction.y})");
                Logger.Log($"Whole Velocity While Jump, speed:{wholeVelocityWhileJumping.speed}, direction: ({wholeVelocityWhileJumping.direction.x}, {wholeVelocityWhileJumping.direction.y})");
                Logger.Log($"Whole Offset, ({wholeOffset.offset.x}, {wholeOffset.offset.y})");
                Logger.Log($"Whole Positon, ({whole.transform.position.x}, {whole.transform.position.y})");
            }
        ...
        void FU_FistSpeed()
        {
            ...
            */
            ...
            {
                anyKeyHold = PlayerInput.lAnyDirKeyHold,
                keyDir = PlayerInput.lMvDir,
                handState = leftFistState,
                moveDir = PlayerInput.lMvDir * (leftFistState == FistState.GrabEnv ? dirReversedEnv : 1)
            };
            ParameterForFistVelocity rightParameter = new ParameterForFistVelocity
            {
                anyKeyHold = PlayerInput.rAnyDirKeyHold,
                keyDir = PlayerInput.rMvDir,
                handState = rightFistState,
                moveDir = PlayerInput.rMvDir * (rightFistState == FistState.GrabEnv ? dirReversedEnv : 1)
            };
        ...
        */
            ...
            leftParams.joint1Pos = armGesture.leftJoint1.transform.localPosition;
            ...
            rightParams.joint1Pos = armGesture.rightJoint1.transform.localPosition;
        ...
        */
        ...
        void FU_Whole_Velocity()
        {
            ...
            if (leftFistState.IsGrabing_Env_StuffEnv() || rightFistState.IsGrabing_Env_StuffEnv())
            {
                ...
                wholeState = WholeState.BeforeJump;
            }
            else
            {
                ...
                Logger.Log($"【before】 Whole Velocity While Jump Before FixedUpdate, speed:{wholeVelocityWhileJumping.speed}, direction: ({wholeVelocityWhileJumping.direction.x}, {wholeVelocityWhileJumping.direction.y})");
                ...
                wholeState = WholeState.WhileJumping;
            }
        }
        ...
        */
            else
            {
                ...
                else
                {
                    ...
                    {
                        /*
                        在FootState出现“左右有些空白”的情况后。运动方便，也需要修改。
                        见：https://www.notion.so/FootState-97ff1a3278824cdfb9e64372a299aa06
                        
                        原来的算法，footState == FootState.EnvRock，只考虑与Ground的碰撞。
                        这本来就有问题。
                            当时的描述为：
                            如果经过一个Air，再出现EnvRock。这里的算法就会有问题。但一帧移动的很少，目前应该不会出现。先不管它。
                        可以认为，除了Ground，总是应该考虑：和Env碰撞的情况，以及没东西碰撞的情况。
                            目前想到，是把line通过collider切割，来看情况。
                            但是，实现有些麻烦。
                        
                        所以，相比早先的算法那，只加入，左右点如果是在空气中的情况。
                        这时，会考虑和Env的碰撞。
                            这么做，和FootState部分的一些条件假设是一致的。
                            见FootState第二版的情况假设。
                    ...
                    */
                        ...
                        //左右点如果在空气中，考虑Rock的碰撞
                        RaycastHit2D hitRockLeft = new RaycastHit2D();
                        RaycastHit2D hitRockRight = new RaycastHit2D();
                        {
                            int layerMaskRock = LayerMask.GetMask("EnvRock");

                            Vector2 pointBL = bottomLeftPoint.transform.position;
                            Collider2D colliderLeft = Physics2D.OverlapPoint(pointBL, layerMaskRock);
                            if (colliderLeft == null)
                            {
                                hitRockLeft = Physics2D.Raycast(pointBL, new Vector2(0, -1), downDis, layerMaskRock);
                            }

                            Vector2 pointBR = bottomRightPoint.transform.position;
                            Collider2D colliderRight = Physics2D.OverlapPoint(pointBR, layerMaskRock);
                            if (colliderRight == null)
                            {
                                hitRockRight = Physics2D.Raycast(pointBR, new Vector2(0, -1), downDis, layerMaskRock);
                            }
                        }
                        ...
                        RaycastHit2D hitGroundRight;
                        {
                            ...
                            Vector2 pointBM = bottomMiddlePoint.transform.position;
                            Vector2 pointBR = bottomRightPoint.transform.position;
                            hitGroundLeft = Physics2D.Raycast(pointBL, new Vector2(0, -1), downDis, layerMaskGround);
                            hitGroundMiddle = Physics2D.Raycast(pointBM, new Vector2(0, -1), downDis, layerMaskGround);
                            hitGroundRight = Physics2D.Raycast(pointBR, new Vector2(0, -1), downDis, layerMaskGround);
                        }


                        if (!hitRockLeft.collider && !hitRockRight.collider
                                                  && !hitGroundLeft.collider && !hitGroundMiddle.collider && !hitGroundRight.collider)
                        {
                            tempWholeOffset = velocity * Time.fixedDeltaTime;
                        }
                        else
                        {
                            float downDisRockL = float.MaxValue;
                            float downDisRockR = float.MaxValue;
                            float downDisBL = float.MaxValue;
                            float downDisBM = float.MaxValue;
                            float downDisBR = float.MaxValue;
                            if (hitRockLeft.collider)
                            {
                                downDisRockL = hitRockLeft.distance;
                            }
                            if (hitRockRight.collider)
                            {
                                downDisRockR = hitRockRight.distance;
                            }
                            if (hitGroundLeft.collider)
                            {
                                downDisBL = hitGroundLeft.distance;
                            }
                            if (hitGroundMiddle.collider)
                            {
                                downDisBM = hitGroundMiddle.distance;
                            }
                            if (hitGroundRight.collider)
                            {
                                downDisBR = hitGroundRight.distance;
                            }
                            downDis = Mathf.Min(downDisRockL, downDisRockR, downDisBL, downDisBM, downDisBR);
