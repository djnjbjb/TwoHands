...
namespace Ludo.TwoHandsWar.THWControllerSub
{
    ...
    public GameObject rightLine2p;
        ...
        public ArmGesture(GameObject whole)
        {
        ...
        */
            ...
            */
            ...
            else if (b + c > a && !Geometry.FloatEqual_WithIn0p001(b+c, a))
            {
                ...
                值得注意的是，这里差不多的计算。和其他的差不多用通用的阈值（Ludo.Utility.FloatEqual0p001）可能不合适，但先这样试一下。
            ...
            */
                ...
                Vector2 joint1ToJoint2_pointOnLeft = Geometry.Vec2Rotate(joint1ToFist.normalized, -angleJoint1) * length1;
                joint2Pos_pointOnLeft = joint1Pos + joint1ToJoint2_pointOnLeft;
                Vector2 joint2Pos_pointOnRight;
                Vector2 joint1ToJoint2_pointOnRight = Geometry.Vec2Rotate(joint1ToFist.normalized, angleJoint1) * length1;
                ...
                if (currentLR) Logger.Log($"Current == Triangle", "Temp");
                if (currentLR) Logger.Log($"Pre:({joint2PosPre.x},{joint2PosPre.y})", "Temp");
                if (currentLR) Logger.Log($"Left:({joint2Pos_pointOnLeft.x},{joint2Pos_pointOnLeft.y})", "Temp");
                if (currentLR) Logger.Log($"Right:({joint2Pos_pointOnRight.x},{joint2Pos_pointOnRight.y})", "Temp");
                ...
                if (joint2StatePre == Joint2State.Line)
                {
                    ...
                    if (currentLR) Logger.Log("Pre == Line", "Temp");
                }
                /*
            ...
            */
                else if (joint2StatePre == Joint2State.Singularity)
                {
                    ...
                    if (currentLR) Logger.Log("Pre == Singularity", "Temp");
                ...
                else if (joint2StatePre.IsTriangle())
                {
                    ...
                    先拍脑门定一个。
                */
                    if (currentLR) Logger.Log("Pre == Triangle", "Temp");
                    /*
                    如何判断是否经过极值状态？
                ...
                */
                    ...
                    {
                        Vector2 nearestPoint = Geometry.FindNearestPointOnLine(fistPosPre, fistPos - fistPosPre, joint1Pos);
                        if (currentLR) Logger.Log($"nearestPoint == {nearestPoint}", "Temp");
                        if ((joint1Pos - nearestPoint).magnitude <= singularityRadius_ifPassSigularity)
                        {
                            ...
                            if (currentLR) Logger.Log($"line1 == {line1}", "Temp");
                            if (currentLR) Logger.Log($"line2 == {line2}", "Temp");
                    ...
                    if (currentLR) Logger.Log($"byPassSingularity == {byPassSingularity}", "Temp");
                    ...
                    else
                    {
                        ...
                        if (currentLR) Logger.Log($"distanceLeft:{distanceLeft}", "Temp");
                        if (currentLR) Logger.Log($"distanceRight:{distanceRight}", "Temp");
                    }
            ...
            else
            {
                ...
                if (currentLR) Logger.Log("Current == Line", "Temp");
            }
        }
        ...
        static void GetShapeRightOther (
            ...
            out Quaternion fistRotation)
        {
            if (currentLR) Logger.Log($"joint2Pos:({joint2Pos.x},{joint2Pos.y})", "Temp");
            ...
            line1Rotation = new Quaternion();
            {
                ...
                if (currentLR) Logger.Log($"joint1ToJoint2:({joint1ToJoint2.x},{joint1ToJoint2.y})", "Temp");
                if (currentLR) Logger.Log($"angleLine1:{angleLine1}", "Temp");
                line1Rotation = Quaternion.Euler(0, 0, angleLine1);
                if (currentLR) Logger.Log($"line1Rotation:{line1Rotation}", "Temp");
            }
            ...
            fistRotation = new Quaternion();
            {
                ...
                if (currentLR) Logger.Log($"joint2ToFist:({joint2ToFist.x},{joint2ToFist.y})", "Temp");
                if (currentLR) Logger.Log($"angleLine2:{angleLine2}", "Temp");
                line2Rotation = Quaternion.Euler(0, 0, angleLine2);
                if (currentLR) Logger.Log($"line1Rotation:{line2Rotation}", "Temp");
