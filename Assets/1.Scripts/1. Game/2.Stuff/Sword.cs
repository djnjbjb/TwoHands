        ...
        */
            ...
            if (Time.fixedTime - soundEffect_lastWhooshTime >= soundEffect_whooshInterval)
            {
                ...
                */
                if (states.state == State.WithHand && states.withHandState == WithHandState.Free)
                {
                    ...
                    if (allNotZero)
                    {
                        var audio = THWController.singleton.hcAudio;
                        //随机音效，但保证前后两次不重复。
            ...
            if (states.state == State.WithHand)
            {
                ...
                */
                ...
                GameObject fistObject = null;
                {
                    if (firstFist != null)
                    {
                        THWController.ValueForSword value = THWController.singleton.ForSword_GiveValue(firstFist);
        ...
        void CheckAndRemoveFist()
        {
            ...
            foreach (string fist in fists)
            {
                if (fist != firstFist)
                {
                    if (!THWController.singleton.ForSword_CheckOverlap(fist, this.gameObject))
                    {
                        deleteList.Add(fist);
                    }
                }
            ...
            foreach (string deleteFist in deleteList)
            {
                ...
                THWController.singleton.ForSword_UnSetGrabedStuff(deleteFist);
            }
        }

        public void ThrowSword(float speed, DirectionOf9History offsetHistoryOfNineDir)
        {
            Logger.Log("ThrowSword", "Temp");
            /*
                先判断有无Collier
                由此决定State
            */
            ...
            if (states.noHandState == NoHandState.Fly)
            {
                Logger.Log("ThrowSword, Fly", "Temp");
            }
            if (states.noHandState == NoHandState.Env)
            {
                Logger.Log("ThrowSword, Env", "Temp");
            }
